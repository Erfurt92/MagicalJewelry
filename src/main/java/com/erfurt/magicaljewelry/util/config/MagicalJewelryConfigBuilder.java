...
public class MagicalJewelryConfigBuilder
{
	...
	static class CommonConfig
	{
		CommonConfig(ForgeConfigSpec.Builder config_builder)
		{
			...
			JEWEL_EPIC_DROP_RATE = config_builder.comment("Note that to change the drop rate for Uncommon rarity, you have to change the other values.", "Drop rate for Epic rarity [default: 15]").defineInRange("epicDrop", 15, 5, 30);
			JEWEL_LEGENDARY_DROP_RATE = config_builder.comment("Note that to change the drop rate for Uncommon rarity, you have to change the other values.", "Drop rate for Legendary rarity [default: 5]").defineInRange("legendaryDrop", 5, 1, 10);
			...
			JEWEL_LEGENDARY_UPGRADE_ONLY = config_builder.comment("Should Legendary rarity be upgrade only, if upgrading is disabled, you can't get Legendary rarity").define("legendaryUpgradeOnly", false);
			JEWEL_UPGRADE_DISABLE = config_builder.comment("Should Jewels be able to be upgraded").define("disableUpgrading", false);
			config_builder.pop();
